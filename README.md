...
The site reads `papers/` files  live from GitHub and writes them back through the 
GitHub Contents API, so nothing is generated, nothing is cached server-side, 
and nothing can go stale. Editing a `.md` file directly on GitHub and using the 
site produce identical results.
...
- **Add a paper** — fill in the form, or **Import** a `.bib`, `.ris`, or
  ...
  duplicates are flagged and unticked before you commit anything. You may also
  manually add papers with the **Add paper** button.
  **Edit** you can edit on any citation to add notes, update tags, or any other
  relavent information. Do this by clicking on the citation you want to edit and
...
No need for a token to read, to add or edit you need a GitHub token from an account that is a
collaborator here. 
...
Unknown fields are rejected rather than dropped. Parsing a file and re-writing it
